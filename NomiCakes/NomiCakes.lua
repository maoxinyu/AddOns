				...
				if ingredient and recipeList then -- and recipes then
					...
					if unlearned ~= 0 then
						if canLearn ~= 0 then
							...
							text = format('可学配方:%d |T%d:16|t [%s] x%d', canLearn, ingredientIcon, ingredientName, count)
						else
						...
						text = format('|cff660000可学配方:0 |T%d:16|t [%s]', ingredientIcon, ingredientName)
					end
					if text then
...
local function checknomiBtn()	--bf@178.com
	if not NomiCakesBtn then
		CreateFrame("Button", "NomiCakesBtn", GossipFrame, "UIPanelButtonTemplate");
		NomiCakesBtn:SetPoint("TOPLEFT", GossipFrame, "TOPLEFT", 66, -33);
		NomiCakesBtn:SetSize(88, 24)
		NomiCakesBtn:SetText("食谱查询")
		NomiCakesBtn:SetScript('OnLeave', GameTooltip_Hide)
		NomiCakesBtn:SetScript('OnEnter', function(self)
			GameTooltip:SetOwner(self, 'ANCHOR_RIGHT')
			GameTooltip:SetText("也可以使用命令查询(/nomi)")
			GameTooltip:Show()
		end)
		NomiCakesBtn:SetScript('OnClick', function()
			SlashCmdList.NOMICAKES()
		end)
	end
	if IsNomi then
		NomiCakesBtn:Show();
